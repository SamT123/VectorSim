...
// [[Rcpp::export]]
List test_fn_list(Rcpp::List l, int len){
  for (int i = 0; i<len; i++){
    l[i] = l[i] + 3
  }
  return l;
...
// arma::vec rmn(unsigned int N, arma::vec p, gsl_rng* r){
...
// arma::vec gsl_mmm_1(arma::vec N, arma::mat P, gsl_rng* r){
//     size_t K = N.size();
//     int i;
//     arma::vec x(K);
//     for(i=0; i<K; i++){
...
// // [[Rcpp::export]]
// arma::mat gsl_mmm(arma::mat X_, arma::mat P){
//     int j;
//     gsl_rng * r = gsl_rng_alloc (gsl_rng_mt19937);
//     long seed = rand()/(((double)RAND_MAX + 1)/10000000) * getpid();
//     gsl_rng_set (r, seed);
//     arma::mat X(X_.n_rows, X_.n_cols);
//     for(j=0; j<X.n_cols; j++){
//         X.col(j) = gsl_mmm_1(X_.col(j), P, r);
//     }
//     gsl_rng_free (r);
//     return X;
// }
...
//     r_pre[,i+1,] <- D_pre[,i+1,]**2 / (pA_pre[,i+1,]*(1-pA_pre[,i+1,])*pB_pre[,i+1,]*(1-pB_pre[,i+1,]))
//     }
//   return(list(pAB_pre=pAB_pre, pAb_pre=pAb_pre, paB_pre=paB_pre, pab_pre=pab_pre, pAB_post=pAB_post, pAb_post=pAb_post, paB_post=paB_post, pab_post=pab_post, D_pre=D_pre, D_post=D_post, pA_pre=pA_pre, pA_post=pA_post, pB_pre=pB_pre, pB_post=pB_post, r_pre=r_pre, r_post = r_post, loci = loci, t_tot = NA, t_curr = tmin+t_add, n_subpops = n_subpops))
// }




// list(pAB_pre=pAB_pre, pAb_pre=pAb_pre, paB_pre=paB_pre, pab_pre=pab_pre, pAB_post=pAB_post, pAb_post=pAb_post, paB_post=paB_post, pab_post=pab_post, D_pre=D_pre, D_post=D_post, pA_pre=pA_pre, pA_post=pA_post, pB_pre=pB_pre, pB_post=pB_post, r_pre=r_pre, r_post = r_post, loci = loci, t_tot = NA, t_curr = tmin+t_add, n_subpops = n_subpops))
// }
// 
// population_list, N_v = c(1000, 1000), M = array(c(.9,.1,.1,.9), dim = c(2,2)), c=0.5, t_add = 100